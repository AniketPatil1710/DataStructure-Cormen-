...
status_t pop(queue_t* p_queue , queue_data_t* pop_data);
...
/*BST Interface Routines Declaration*/
...
void bfs_tree(bst_t* p_bst)
{
    ...
    //queue_data_t p_bst_node = p_bst->p_root_node ;
...
status_t back(queue_t* p_queue , queue_data_t* new_data)
{
    return(pop_beg(p_queue , new_data));
}
...
bool is_list_empty(list_t* p_list)
{
    return(p_list->next==p_list && p_list==p_list);
}
